    ...
    if (is_lbk_chr(lxr.lka_chr())) {
      ...
      if (VLD(SYM_LIT_LBK)) {
        ...
        if (lit_brk == NULL_PTR) {
          ...
          const size_t blk_dlm_cnt = blk_dlms.size();
          if (scn_eol(lxr, blk_dlms, blk_ctx_stk)) {
            end_idx = lbk_bgn_idx;
          } else if (blk_dlms.size() > blk_dlm_cnt && blk_dlms.back().sym() == SYM_LIT_LBK) {
            lxr.jmp_pos(blk_dlms.back().end_pos());
          } else {
            // Reached when scn_eol recorded no new delimiter -- either because
            // it produced none, or because the list is at the ceiling that
            // keeps the scanner state serializable (an inline span covering
            // more rows than 1024 bytes of state can describe).  The old code
            // read blk_dlms.back() without checking that it was the entry
            // scn_eol had just made: with nothing appended that is a delimiter
            // from an earlier row, and jumping to its end position moves the
            // lexer BACKWARDS, so the same line break is scanned for ever.
            // Abandoning the span instead reads the construct as literal text,
            // which is the escape this branch already takes for a line break it
            // cannot use, and it always makes progress.
            is_invalid = true;
          }
