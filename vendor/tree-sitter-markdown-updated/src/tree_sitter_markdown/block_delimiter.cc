...
unsigned BlockDelimiterList::serialized_size() const {
  return 1 + BLK_DLM_SRL_SIZE * list_.size();
}
...
BlockDelimiterList::BlockDelimiterList(): bgt_(NULL_PTR) {}
void BlockDelimiterList::set_budget(const SerializationBudget *bgt) { bgt_ = bgt; }

void BlockDelimiterList::pop_front() { list_.pop_front(); }
bool BlockDelimiterList::is_full() const {
  return list_.size() >= SRL_MAX_COUNT_PER_LIST
      || !srl_budget_has_room(bgt_, BLK_DLM_SRL_SIZE);
}
// A document's own structure accumulates here.  Dropping the delimiter is what
// keeps the state serializable; see serialization_limit.h.
void BlockDelimiterList::push_back(const BlockDelimiter &dlm) {
  if (is_full()) return;
  list_.push_back(dlm);
}
// insert()'s one caller (the link-reference-definition handler in
// inline_scan.cc) increments the iterator it gets back, so a refusal has to
// hand back something that can still be incremented: the insertion point
// itself, which is also what the caller's `++insert(...)` would have yielded
// had the new element been placed immediately before it.
BlockDelimiterList::Iterator BlockDelimiterList::insert(const BlockDelimiterList::Iterator itr, const BlockDelimiter &dlm) {
  if (is_full()) return itr;
