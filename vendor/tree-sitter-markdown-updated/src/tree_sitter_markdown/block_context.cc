...
unsigned BlockContextStack::serialized_size() const {
  return 1 + BLK_CTX_SRL_SIZE * stk_.size();
}
...
BlockContextStack::BlockContextStack(): bgt_(NULL_PTR) {}
void BlockContextStack::set_budget(const SerializationBudget *bgt) { bgt_ = bgt; }

// Full when one more context would no longer serialize, or when the one-byte
// count that precedes the entries could no longer hold the length.
bool BlockContextStack::is_full() const {
  return stk_.size() >= SRL_MAX_COUNT_PER_LIST
      || !srl_budget_has_room(bgt_, BLK_CTX_SRL_SIZE);
}
// The callers in scanner.cc test is_full() before they commit to opening a
// block, so a refusal here is the last line of defence rather than the usual
// path.
bool BlockContextStack::push(const BlockContext &ctx) {
