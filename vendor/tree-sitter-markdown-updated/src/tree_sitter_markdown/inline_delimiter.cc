...
MinimizedInlineDelimiterList::MinimizedInlineDelimiterList(): bgt_(NULL_PTR) {}
void MinimizedInlineDelimiterList::set_budget(const SerializationBudget *bgt) { bgt_ = bgt; }
bool MinimizedInlineDelimiterList::is_full() const {
  return list_.size() >= SRL_MAX_COUNT_PER_LIST
      || !srl_budget_has_room(bgt_, MIN_INL_DLM_SRL_SIZE);
}
// Inline delimiters arrive in bulk from InlineDelimiterList::transfer_to, which
// is where a single very long line of link text overran the buffer -- and, past
// 255 entries, wrapped the one-byte count this list writes.  Past the limit the
// surplus delimiters are dropped, so emphasis beyond that point on such a line
// is read as literal text rather than as markup.
...
unsigned MinimizedInlineDelimiterList::serialized_size() const {
  return 1 + MIN_INL_DLM_SRL_SIZE * list_.size();
}
